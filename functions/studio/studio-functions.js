...
{    
    ...
    if (typeof(str) === 'string'){
        ...
        str = String(str);
        reverseStr = Number(str.split('').reverse().join(''));
    }    
    return reverseStr;
}
...
function completeReversal(arr)
{
    ...
    {
        reverseArr.push(reverseCharacters(arr[i]));
    }
...
function funPhrase(origionalStr)
{
    modifiedStr = '';
    let output = '';
    if(str.length <= 3)
    {
        modifiedStr = str.slice(str.length-1);
    } else {
        modifiedStr = str.slice(0,3);
    }
    output = `We put the '${modifiedStr}' in '${origionalStr}'.`;
    return output;
}
// Test Function
